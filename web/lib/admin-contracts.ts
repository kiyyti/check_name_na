import {z} from 'zod';
import {classLevels} from './class-levels.mjs';
...
const sessionLevels=z.string().trim().max(2000).refine(value=>value===''||value.split(',').every(level=>level.trim().length>0&&level.trim().length<=50)&&classLevels(value).length<=30,'กรุณาเลือกระดับชั้นให้ถูกต้อง');
...
export const sessionInput=z.object({session_id:short,session_number:z.string().regex(/^[1-9]\d{0,4}$/),topic:short,group_name:short,class_level:sessionLevels,starts_at:datetime,ends_at:datetime,checkin_opens_at:datetime,checkin_closes_at:datetime,late_at:datetime,absence_at:datetime,status:z.enum(['draft','open','closed','cancelled']),room_name:id,latitude:numeric,longitude:numeric,radius_m:numeric,max_accuracy_m:numeric,_version:version}).strict();
...
export const adminGatewaySchema=z.object({ok:z.boolean(),code:z.string().optional(),message:z.string().optional(),retryAfter:z.number().optional(),data:adminDataSchema.optional(),capabilities:z.object({multiLevelSessions:z.boolean()}).optional()});
