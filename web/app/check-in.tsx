...
type Config={ready:boolean;message?:string;levels:string[];room?:string;course?:string;lateTime?:string;absenceTime?:string};
...
export default function CheckIn(){
 ...
 return <main className="check-shell">
  ...
  <div className="schedule"><div className="schedule-title"><Clock3 size={19}/><h2>เวลาเช็คชื่อ</h2><span>เวลาไทย</span></div><div className="time-row"><span className="status-tag present">มาเรียน</span><strong>ก่อน {config?.lateTime||'08:30'}</strong></div><div className="time-row"><span className="status-tag late">มาสาย</span><strong>{config?.lateTime||'08:30'} – ก่อน {config?.absenceTime||'09:00'}</strong></div><div className="time-row"><span className="status-tag absent">ขาดเรียน</span><strong>ตั้งแต่ {config?.absenceTime||'09:00'}</strong></div></div>
