...
type Config={ready:boolean;message?:string;levels:string[];room?:string;course?:string;lateTime?:string;absenceTime?:string;teacherName?:string;locationName?:string;schedules?:{label:string;room:string;lateTime:string;absenceTime:string}[]};
...
export default function CheckIn(){
 ...
 function resetResult(){setReceipt(null);setError('');setLocationAccuracy(null);requestId.current='';}
 async function submit(e:React.FormEvent){
  ...
  busy.current=true;setError('');setReceipt(null);setLocationAccuracy(null);setPhase('กำลังตรวจพิกัด');
  try{
   ...
   if(Number.isFinite(p.coords.accuracy))setLocationAccuracy(p.coords.accuracy);
 ...
 return <main className="check-shell">
  ...
  <div className="check-layout"><section className="intro"><p className="eyebrow">{config?.locationName||'วิทยาลัยการอาชีพเถิน · ลำปาง'}</p><h1>พร้อมเรียนแล้ว<br/><span>เช็คชื่อกันนะ</span></h1><p className="teacher"><GraduationCap size={22}/>{config?.teacherName||'อาจารย์ปภังกร บุตรศรี'}</p><p className="intro-copy">กรอกข้อมูลของตัวเอง แล้วอนุญาตตำแหน่ง<br className="desktop-break"/>เพื่อยืนยันว่าคุณอยู่ในพื้นที่เรียน</p>
  {(config?.schedules?.length?config.schedules:[{label:'',room:config?.room||'',lateTime:config?.lateTime||'08:30',absenceTime:config?.absenceTime||'09:00'}]).map((schedule,index)=><div className="schedule" key={index}><div className="schedule-title"><Clock3 size={19}/><h2>เวลาเช็คชื่อ{schedule.label?' · '+schedule.label:''}</h2><span>เวลาไทย</span></div><div className="time-row"><span className="status-tag present">มาเรียน</span><strong>ก่อน {schedule.lateTime}</strong></div><div className="time-row"><span className="status-tag late">มาสาย</span><strong>{schedule.lateTime} – ก่อน {schedule.absenceTime}</strong></div><div className="time-row"><span className="status-tag absent">ขาดเรียน</span><strong>ตั้งแต่ {schedule.absenceTime}</strong></div>{(config?.schedules?.length||0)>1&&<p className="field-hint">{schedule.room}</p>}</div>)}
  <div className="location-note"><MapPin size={21}/><div><b>{(config?.schedules?.length||0)>1?'ตรวจสถานที่ตามรอบเรียนของคุณ':config?.room||'ห้องเรียนอาจารย์ปภังกร บุตรศรี'}</b><p>{config?.room?'เช็คชื่อได้ภายในรัศมีพื้นที่เรียนที่กำหนด':'รอระบุชื่อห้องและขอบเขตพื้นที่เช็คชื่อ'}</p></div></div></section>
  <section className="form-card" aria-labelledby="form-title"><div className="card-heading"><span className="step-number">01</span><span>ข้อมูลผู้เรียน</span><BookOpen size={21}/></div><h2 id="form-title">วันนี้มาเรียนแล้ว</h2><p className="card-description">{config?.course||'เช็คชื่อสำหรับการเรียนครั้งนี้'}</p>
   ...
   <p className="field-hint" role="status">{locationAccuracy!==null?`ความคลาดเคลื่อนตำแหน่งครั้งนี้: ประมาณ ${Math.ceil(locationAccuracy).toLocaleString('th-TH')} เมตร (ยิ่งน้อยยิ่งแม่นยำ)`:''}</p>
  ...
  <footer className="page-footer"><span>check name na</span><span>หนึ่งวิชา · ทุกครั้งที่มาเรียน</span><a href="/qr">QR สำหรับเข้าเว็บไซต์</a><a href="/admin">สำหรับอาจารย์</a></footer>
 </main>;
}
